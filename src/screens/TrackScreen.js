...
import { COST_PER_MILE, IRS_RATE_PER_MILE } from '../utils/constants'; // shared rate constants
...
export default function TrackScreen({ navigation }) {
  ...
  // FIX #1: Use a ref to hold the latest gigEarnings so the watcher closure always reads current value
    ...
    if (user) {
      getDoc(doc(db, 'users', user.uid)).then((snap) => {
        if (snap.exists()) {
          ...
          // FIX #5: Load home coords — matches field names written by SettingsScreen (homeLat/homeLon)
        ...
        <View style={styles.profitCard}>
          ...
          <View style={styles.inputRow}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                setGigEarnings(text);
                gigEarningsRef.current = text; // FIX #1: Keep ref in sync on manual edits too
